...
class Publication: NSManagedObject {
 ...
 //   var wasUpdated: Bool = false
    ...
    var photoUrl: String { get {
        ...
        return "\(self.uniqueId!.integerValue).\(self.version!.integerValue).jpg"}
    }
    ...
    func updateAfterUserCreation(params: [String: AnyObject], context: NSManagedObjectContext) {
        ...
        let id = params[kPublicationUniqueIdKey] as? Int ?? 0
        self.uniqueId = NSNumber(integer: id)
        let version = params[kPublicationAudianceKey] as? Int ?? 1
        self.audiance = NSNumber(integer: version)
