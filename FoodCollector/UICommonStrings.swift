//
//  UICommonStrings.swift
//  FoodCollector
//
//  Created by Boris Tsigelman on 11.12.2015.
//  Copyright © 2015 Foodonet. All rights reserved.
//

import UIKit


// Button labels
let kYesButtonTitle = NSLocalizedString("Yes", comment:"Title for an alert button")
let kOKButtonTitle = NSLocalizedString("OK", comment:"Title for an alert button")
let kDoneButtonTitle = NSLocalizedString("Done", comment:"Title for an alert button")
let kNoButtonTitle = NSLocalizedString("No", comment:"Title for an alert button")
let kCancelButtonTitle = NSLocalizedString("Cancel", comment:"Title for an alert button")
let kOopsAlertTitle = NSLocalizedString("Oops...", comment:"Title for an alert message")
let kBackButtonTitle = NSLocalizedString("Back", comment:"Title for a navigation button")


//Alert Message Titles and Bodies
let kPhoneNumberIncorrectAlertMessage = NSLocalizedString("It seems the phone number is incorrect. Please check you have typed it correctly.", comment:"Alert message body")
let kSendSMSfailedAlertTitle = NSLocalizedString("Message delivery failed", comment:"Alert title")
let kSendSMSTryAgainAlertMessage = NSLocalizedString("Try again?", comment:"Alert message body")

class UICommonStrings: NSObject {

}
